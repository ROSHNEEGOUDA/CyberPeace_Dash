...
function Home() {
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      ...
      setDiscussions(prevDiscussions => [...prevDiscussions, { id: docRef.id, ...newCourse }]);
  ...
  const deleteCourse = async () => {
    if (selectedCourseId !== null) {
      try {
        ...
        setDiscussions(prevDiscussions => prevDiscussions.filter(course => course.id !== selectedCourseId));
      ...
      <div className="container mx-auto px-4 relative z-0 flex flex-col justify-center items-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mt-2 md:mt-3">
          {discussions.map((discussion, index) => (
            <Link to={`/${discussion.id}`} key={index} className="block">
              <div className="overflow-hidden bg-blue-300 shadow-md rounded-md transition duration-300 ease-in-out transform hover:scale-105 group">
                <img src={discussion.imageUrl} alt={discussion.title} className="w-full h-48 object-cover" />
                <div className="p-4">
